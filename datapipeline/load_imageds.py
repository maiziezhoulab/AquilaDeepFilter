...
class LoadData(object):
    ...
    def load_labels(self):
        ...
        self.all_images_path = [str(path) for path in self.path_to_dir.glob("*/*")]
    ...
    def create_dataset(self,
        ...
        labels_dataset = tf.data.Dataset.from_tensor_slices(self.all_images_labels)

        # develop an image dataset
        image_dataset = tf.data.Dataset.from_tensor_slices(self.all_images_path)

        # process the image dataset
        image_dataset = image_dataset.map(self.process_image, num_parallel_calls=autotune)
